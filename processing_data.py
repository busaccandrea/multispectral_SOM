...
def nnmf_based(data, smoothing=0, n_components_=15, write_to_file=False, max_iterations=200):
    ...
    H = nmf.components_
    if write_to_file:
        np.save(results_base_folder + 'nmf/W_' + str(n_components_) + '_comp_smth_' + str(smoothing) + '.npy', W)
        np.save(results_base_folder + 'nmf/H_' + str(n_components_) + '_comp_smth_' + str(smoothing) + '.npy', H)
    
    return [W,H]


def data_clustering(distance_matrix):
    h, w = distance_matrix.shape
    distance_matrix = distance_matrix.reshape(h * w)
    perc = 1/distance_matrix
    sum_ = np.sum(perc)
    for i in range(len(distance_matrix)):
        distance_matrix[i] = perc[i]/sum_
    return distance_matrix


def data_clustering_cmeans(distance_matrix):
    perc = 1/distance_matrix
    sum_ = np.sum(perc)
    for i in range(len(distance_matrix)):
        distance_matrix[i] = perc[i]/sum_
