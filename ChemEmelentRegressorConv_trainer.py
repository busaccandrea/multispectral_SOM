...
if __name__ == '__main__':
    ...
    data = np.load('data/DOggionoGiulia/EDF/data.npy')
    # elements = [
    #     1,
    #     2,
    #     3,
    ...
    for i, element in enumerate(elements):
        ...
        csvfile = 'data/DOggionoGiulia/' + str(element) + '.csv'
        ...
        # test_set, training_set = random_split(el_dataset, [testsize, trainsize]) #PROBLEMA: RITORNA UN SUBSET
        ...
        train_regressor(model, training_set, test_set, epochs=epochs, batch_size=batch_size, lr=learning_rates[i], sampler={'training':training_sampler, 'test':test_sampler}, model_filename=str(element)+'conv.p')
        print('\nDone.')